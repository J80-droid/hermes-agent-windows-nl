...
function Install-Uv {
    ...
    # Check common install locations before managed install
    $uvPaths = @(
...
# at the top to populate $script:UvCmd from managed location, PATH, or known paths.
# Throws if uv is not findable — the caller's stage then surfaces a clean error.
function Resolve-UvCmd {
    ...
    # Fall back to PATH (winget, manual installs, %USERPROFILE%\.local\bin).
...
function Install-Dependencies {
    ...
    # Re-pin UV_PYTHON to the venv interpreter (cross-process bootstrap stages).
    if (-not $NoVenv) {
        $venvPythonExe = Join-Path $InstallDir "venv\Scripts\python.exe"
        if (Test-Path $venvPythonExe) {
            $env:UV_PYTHON = $venvPythonExe
