...
| Component/Keuze/Status op één regel met em-dash | Model levert `Component: … Keuze: … Status: … —————— Component: …` | `_parse_collapsed_record_rows`; ook **zonder `##`** (onder `**Label:**`); `Laag`/`Wat`/`Waarom` idem; SOUL verbiedt em-dash-muur; `RUN_COLLAPSED_RECORD_PSEUDO_TABLE_E2E.bat`; na deploy `SYNC_SOUL_SNIPPETS.bat` + `/new` |
