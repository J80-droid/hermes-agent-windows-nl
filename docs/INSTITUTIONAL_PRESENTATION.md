...
**E2E audit (pseudo-tabel):** stap **2h** in `RUN_INSTITUTIONAL_E2E.ps1` (10/10; pytest + TS parity + verify). Los: `windows\audits\RUN_PSEUDO_TABLE_NORMALIZER_E2E.bat`. **Context-aware overview:** `windows\audits\RUN_CONTEXT_AWARE_PSEUDO_TABLE_E2E.bat` (10/10; grouped auxiliary + architectuur-stap). **Collapsed record (dedicated):** `audits\RUN_COLLAPSED_RECORD_PSEUDO_TABLE_E2E.bat` (10/10; eligibility, pipe-escape, TS parity). Unit: `pytest tests/hermes_cli/test_collapsed_record_pseudo_table.py`. Gecombineerd: `RUN_AUDITS.bat -IncludePseudoTableNormalizerE2E` of `-IncludeAllE2E`. **Runtime:** `windows\APPLY_INSTITUTIONAL_RUNTIME.bat` voert SOUL-sync + volledige institutioneel E2E (incl. 2h) uit.
...
| Component/Keuze/Status op één regel met em-dash | Model levert `Component: … Keuze: … Status: … —————— Component: …` | `_parse_collapsed_record_rows`; SOUL verbiedt em-dash-muur; `RUN_COLLAPSED_RECORD_PSEUDO_TABLE_E2E.bat`; `pytest tests/hermes_cli/test_collapsed_record_pseudo_table.py`; na deploy `SYNC_SOUL_SNIPPETS.bat` + `/new` |
