...
**Pipeline E2E (2026-05-27):** `audits/RUN_INSTITUTIONAL_PIPELINE_E2E.bat` (11/11) — normalize-contract, compact check, streaming-invarianten, pytest `test_render_pipeline_contract.py`. Unit: `pytest tests/audits/test_institutional_pipeline_e2e_harness.py tests/overlay/test_institutional_render_helpers.py -q`. Bench (lokaal): `python scripts/bench_normalize_markdown.py`.
...
**Verify (pseudo-tabel):** `python scripts/verify_pseudo_table_normalizer.py --verify` — Ollama-vs, auxiliary Cloud/Lokaal, pipe-divider, 4-koloms auxiliary-overzicht, **architectuur-probe** (Component/Keuze/Status em-dash).

**E2E audit (pseudo-tabel):** stap **2h** in `RUN_INSTITUTIONAL_E2E.ps1` (10/10; pytest + TS parity + verify). Los: `windows\audits\RUN_PSEUDO_TABLE_NORMALIZER_E2E.bat`. **Context-aware overview:** `windows\audits\RUN_CONTEXT_AWARE_PSEUDO_TABLE_E2E.bat` (10/10; grouped auxiliary + architectuur-stap). **Collapsed record (dedicated):** `audits\RUN_COLLAPSED_RECORD_PSEUDO_TABLE_E2E.bat` (10/10; eligibility, pipe-escape, TS parity). Unit: `pytest tests/overlay/test_collapsed_record_pseudo_table.py`. Gecombineerd: `RUN_AUDITS.bat -IncludePseudoTableNormalizerE2E` of `-IncludeAllE2E`. **Runtime:** `windows\APPLY_INSTITUTIONAL_RUNTIME.bat` voert SOUL-sync + volledige institutioneel E2E (incl. 2h) uit.
...
| Component/Keuze/Status op één regel met em-dash | Model levert `Component: … Keuze: … Status: … —————— Component: …` | `_parse_collapsed_record_rows`; ook **zonder `##`** (onder `**Label:**`); `Laag`/`Wat`/`Waarom` idem; **SOUL:** schrijf `### Veerkrachtstrategie` + tabel (`SOUL_SHARED_OUTPUT_FORMAT.md`); normalizer is fallback; `SYNC_SOUL_SNIPPETS.bat` + `/new` |
