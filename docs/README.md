...
- **Statusbalk throughput (tok/s):** `display.show_status_bar_tps` (default aan); classic CLI + TUI na cost-segment; `/tps`; module `hermes_cli/status_bar_throughput.py` + `ui-tui/src/domain/statusBarThroughput.ts`; E2E `audits\RUN_STATUS_BAR_THROUGHPUT_E2E.bat`; unit `pytest tests/hermes_cli/test_status_bar_throughput.py`
