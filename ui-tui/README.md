...
- **Layout:** `ComposerPane` uses `paddingX={1}`; `statusRuleColumns()` subtracts 2 cols (non-finite `cols` clamp to 1). `StatusRule` first calls `statusRuleWidths(ruleCols, cwdLabel)` using shared `statusRuleMinLeftWidth()` (8 cols when effective width ≥24, else 1), then `resolveStatusRuleLayout({ cwdReserve: rightWidth + separatorWidth, leftWidth, … })` so cost-tier width uses the **actual** left box width minus a fixed reserve for status/model — not UTF-16 `cwdLabel.length` (CJK-safe via Ink `stringWidth`). Cost renders **inline after the model** inside the left segment (accent color). Breakdown `%` skips non-finite values. Modules: `src/components/appChrome.tsx` (`statusRuleWidths`), `src/domain/usageCostBar.ts` (`resolveStatusRuleLayout`, `statusRuleMinLeftWidth`). Tests: `src/__tests__/statusRule.test.ts`, `src/__tests__/usageCostBar.test.ts` (63 tests).
