...
- **Layout:** `ComposerPane` uses `paddingX={1}`; `statusRuleColumns()` subtracts 2 cols. `StatusRule` first calls `statusRuleWidths(ruleCols, cwdLabel)` (upstream responsive cwd truncation via Ink `stringWidth`), then `resolveStatusRuleLayout({ cwdReserve: rightWidth + separatorWidth, … })` so cost-tier width matches the **display** columns reserved for cwd — not UTF-16 `cwdLabel.length`. Cost renders **inline after the model** inside the left segment (accent color). Modules: `src/components/appChrome.tsx` (`statusRuleWidths`), `src/domain/usageCostBar.ts` (`resolveStatusRuleLayout`). Tests: `src/__tests__/statusRule.test.ts`, `src/__tests__/usageCostBar.test.ts`.
