# Institutional pipeline E2E (normalize contract, render, score, streaming invariants).
param(
    [string]$RepoRoot = ''
)
...
$harness = Join-Path $scriptRoot 'InstitutionalPipelineE2E.harness.py'
if (-not (Test-Path -LiteralPath $harness)) {
    Write-Host '[FAIL] InstitutionalPipelineE2E.harness.py ontbreekt' -ForegroundColor Red
    exit 1
}

$python = Get-HermesAuditPython
Write-Host "=== InstitutionalPipelineE2E (python: $python) ===" -ForegroundColor Cyan
