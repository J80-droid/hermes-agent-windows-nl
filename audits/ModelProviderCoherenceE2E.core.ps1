# Model/provider coherence E2E — atomic persist, detect, repair (no live API).
param(
    [string]$RepoRoot = ''
)
...
$harness = Join-Path $scriptRoot 'ModelProviderCoherenceE2E.harness.py'
if (-not (Test-Path -LiteralPath $harness)) {
    Write-Host '[FAIL] ModelProviderCoherenceE2E.harness.py ontbreekt' -ForegroundColor Red
    exit 1
}

$python = Get-HermesAuditPython
Write-Host "=== ModelProviderCoherenceE2E (python: $python) ===" -ForegroundColor Cyan
