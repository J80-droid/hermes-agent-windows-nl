# Classic CLI pending-input queue E2E (/queue list|pop|clear, hint, status bar).
param(
    [string]$RepoRoot = ''
)
...
$harness = Join-Path $scriptRoot 'CliPendingQueueE2E.harness.py'
if (-not (Test-Path -LiteralPath $harness)) {
    Write-Host '[FAIL] CliPendingQueueE2E.harness.py ontbreekt' -ForegroundColor Red
    exit 1
}

$python = Get-HermesAuditPython
Write-Host "=== CliPendingQueueE2E (python: $python) ===" -ForegroundColor Cyan
