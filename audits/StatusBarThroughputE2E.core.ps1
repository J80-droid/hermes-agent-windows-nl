# Status bar throughput (tok/s) E2E launcher.
param(
    [string]$RepoRoot = ''
)
...
$harness = Join-Path $scriptRoot 'StatusBarThroughputE2E.harness.py'
if (-not (Test-Path -LiteralPath $harness)) {
    Write-Host '[FAIL] StatusBarThroughputE2E.harness.py ontbreekt' -ForegroundColor Red
    exit 1
}

$python = Get-HermesAuditPython
$reportStamp = Get-Date -Format 'yyyy-MM-dd_HHmmss'
$reportPath = Join-Path $scriptRoot ("STATUS_BAR_THROUGHPUT_E2E_REPORT_" + $reportStamp + '.md')
...
[void]$sb.AppendLine("# Status Bar Throughput E2E - $status")
[void]$sb.AppendLine('')
...
if ($exitCode -ne 0) {
    Write-Host '=== STATUS BAR THROUGHPUT E2E: FAIL ===' -ForegroundColor Red
    exit 1
}
Write-Host '=== STATUS BAR THROUGHPUT E2E: PASS ===' -ForegroundColor Green
exit 0
