# RUN_AUDITS 14-fixes E2E — dunne launcher (config-drift + pytest thread + YOLO width).
param(
    [string]$RepoRoot = ''
)
...
$harness = Join-Path $scriptRoot 'RunAudits14FixesE2E.harness.py'
if (-not (Test-Path -LiteralPath $harness)) {
    Write-Host '[FAIL] RunAudits14FixesE2E.harness.py ontbreekt' -ForegroundColor Red
    exit 1
}

$python = Get-HermesAuditPythonLocal
$reportStamp = Get-Date -Format 'yyyy-MM-dd_HHmmss'
$reportPath = Join-Path $scriptRoot ("RUN_AUDITS_14_FIXES_E2E_REPORT_" + $reportStamp + '.md')
$logPath = Join-Path $scriptRoot ("RUN_AUDITS_14_FIXES_E2E_LOG_" + $reportStamp + '.txt')

Write-Host "=== RUN_AUDITS 14-fixes E2E (python: $python) ===" -ForegroundColor Cyan
...
if ($exitCode -ne 0) {
    Write-Host '=== RUN_AUDITS 14-FIXES E2E: FAIL ===' -ForegroundColor Red
    exit $exitCode
}

@"
# RUN_AUDITS 14-fixes E2E — PASS
