...
def main() -> int:
    ...
    _step(
        ...
        "Parallelle invalshoeken" in legal_tpl and "Proactief meedenken" in legal_tpl,
    )
    _step(
        "SOUL legal pushback parallelle tabel",
        "parallelle invalshoeken" in legal_tpl.lower()
        and "Pushback" in legal_tpl,
    )
    _step(
        "SOUL legal example mandaat/disciplinair",
        "mandaat" in legal_tpl.lower() and "disciplinair" in legal_tpl.lower(),
    )

    matters_ex = _read("docs/templates/LEGAL_ACTIVE_MATTERS.example.md")
    _step(
        "MATTERS example GCR + Adjacent checks",
        "GCR 2024-00145" in matters_ex and "Adjacent checks" in matters_ex,
    )
    ...
    _step(
        ...
        "[switch]$Optional" in merge and "Test-IsLegalProfileMemoryUserPath" in merge,
    )
    ...
    _step(
        "pytest test_legal_meta_contract",
        pt.returncode == 0,
        (pt.stderr or pt.stdout or "")[-400:],
    )

    pester = _run_ps1("audits/Invoke-LegalProactiveSparringPester.ps1", timeout=120)
    _step(
    ...
    core = _run_ps1("audits/LegalProactiveSparringE2E.core.ps1", "-RepoRoot", str(REPO))
    _step(
