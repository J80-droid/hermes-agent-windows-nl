...
> **Windows fork:** wijzig **niet** `%LOCALAPPDATA%\\hermes\\config.yaml` of `~/.hermes\\config.yaml` via tools — gebruik `hermes config set` / `hermes model`. Zie `docs/HERMES_HOME_WINDOWS.md`.
