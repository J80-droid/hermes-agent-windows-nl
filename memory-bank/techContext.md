...
- **Hermes config:** root `%LOCALAPPDATA%\hermes\config.yaml` (model); profielen `%LOCALAPPDATA%\hermes\profiles\<naam>\` (MCP, `SOUL.md`, **display**-overlay via `APPLY_TEAM_DISPLAY.bat`, geen `model:`). Secrets + vault: legacy `%USERPROFILE%\.hermes\.env` → `windows/SYNC_HERMES_API_ENV.bat`. **Split-home:** `HermesHomeCommon.ps1`, drift `VERIFY_HERMES_CONFIG_DRIFT.bat`, migratie `APPLY_HERMES_HOME_MIGRATION.bat`, E2E `RUN_HERMES_HOME_E2E.bat` (11 stappen). Runbook: `docs/HERMES_HOME_WINDOWS.md`. Auxiliary: `APPLY_AUXILIARY_HYBRID_PRESET.bat`. Python: `HERMES_WIN_PREFER_LOCALAPPDATA=1`, `hermes config get`.
