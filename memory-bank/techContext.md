...
- **Klassieke CLI statusbalk-kosten:** `hermes_cli/status_bar_cost.py` (Python formatter mirror), `cli.py` hooks (`_append_status_bar_cost_fragments`, `/cost`); `hermes_cli/usage_snapshot.py` (`_seed_agent_session_cost`); Gemini Standard-tier + cache-read in `agent/usage_pricing.py` (`_GOOGLE_GEMINI_PRICING`, route `google-gemini-cli`); E2E `windows/audits/RUN_CLASSIC_CLI_STATUS_BAR_COST_E2E.bat` (12 stappen)
