...
- **TUI statusbalk-kosten (rich):** `hermes_cli/usage_snapshot.py` (gateway usage payload + breakdown), `ui-tui/src/domain/usageCostBar.ts` (formatter/tiers), client-side turn-delta/tools in `createGatewayEventHandler.ts`; team-defaults `show_cost` + `cost_bar_mode`; E2E `windows/audits/RUN_STATUS_BAR_COST_E2E.bat`
