...
- [x] `Invoke-HermesPytestFromManifest.ps1` (thread via audit helper; upstream maxfail/junitxml arg-split fix `448b4041e`)
- [x] stderr teardown-fix + hashtable-splat ExtraArgs + Get-HermesPytestArgsFromConfig (`4af688745` + hardening follow-up)
- [x] `RUN_PYTEST_RUNNER_HARDENING_E2E.bat` 10/10; `HermesPytestFromManifest.Unit.Tests.ps1`; PSSA 0
...
- [x] P1 fork pytest: `RUN_PYTEST_FORK_GATE.bat` (manifest gate; 280 passed, exit 0)
- [x] P1 upstream parity: `RUN_PYTEST_UPSTREAM.bat -ReportOnly` (3529 passed, 50 maxfail Windows-parity; summary lokaal)
...
- [x] Taakbalk-pins doc future-proof (2026-05-30): `windows/TAAKBALK_PINS.md` (drie lagen, rollentabel, mermaid, troubleshooting); harmonisatie START/INSTITUTIONAL/HERMES_START/README/UPSTREAM_SYNC/TERMINAL_WINDOWS/LAUNCH_PROFILES/USER_DATA_OPERATIONS/MEMORY_ARCHITECTURE/ACTIVATION + memory-bank
...
- [x] Bootstrap fast-path (2026-05-30): `launch_bootstrap.json` schema v1, in-process ensure, legacy rag-deps fallback; docs sync; `HERMES_SKIP_LAUNCH_BOOTSTRAP_FAST_PATH=1`
- [x] Model-catalog startup hardening (2026-05-27, 2026-06-01): `model_default_passes_startup_catalog_guard` + `:free`-variant matching; `Test-HermesModelCatalogAvailability` aligned met `validate_requested_model`; pytest `TestStartupCatalogGuard`
- [x] Domein-toolsets mcp+kanban (2026-06-01): alle 14 profielen in `domain_toolsets.yaml`; `mcp`/`kanban` in `CONFIGURABLE_TOOLSETS`; `DOMAIN_TOOLSET_AUDIT.md` + domein-README's bijgewerkt; sync naar runtime profielen
- [x] Launch UI Sink (2026-05-29): `HermesLaunchUi.ps1`, `launch_hermes.ps1`, bat dun; overlap-fix EL `[2K`; bootstrap in orchestrator; E2E `RUN_LAUNCH_UI_SINK_E2E.bat` 8/8; pytest allowlist + unit tests
