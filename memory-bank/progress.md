...
- [x] Statusbalk throughput (tok/s, 2026-05-25): `status_bar_throughput.py` + `statusBarThroughput.ts`; classic CLI + TUI na cost; agent stream/finalize; `/tps` + `show_status_bar_tps`; gateway RPC; `display_markdown` `realign_tables` flag; unit + E2E **11/11** `RUN_STATUS_BAR_THROUGHPUT_E2E`; score-unit `test_score_institutional_render.py` (**53** tests)
