...
**Venice quota (2026-06):** `agent/venice_usage.py` — `/vquota`, `/usage`, statusbalk `VN …`; parallel extended billing/model APIs; tests `tests/agent/test_venice_usage.py`.

**Jatevo custom provider (2026-06-01):** named provider onder `providers.jatevo`; `hermes model` → key-stap (Keep/Replace/Clear), persist naar root runtime + legacy + profiel-`.env`; live `/v1/models` only (geen hardcoded modellen); base URL **`https://jatevo.ai/v1`** (niet `api.jatevo.ai`); template `docs/templates/PROVIDERS_JATEVO.yaml`; doctor/E2E/sync `JATEVO_API_KEY`; **`/jquota`** + **`/usage`** (dashboard `0/N`); statusbalk **`JV used/max`** (90s cache); HTTP **429** → hint `/jquota` + 00:00 UTC (`agent/jatevo_usage.py`).
