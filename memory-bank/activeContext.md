...
**Memory L1–L4 productieniveau (2026-05-24):** vault = `Documents/Hermes Knowledge`; geen L3. **`SYNC_TRUST_RUNTIME.bat`** = volledige keten incl. dedup (preamble + mojibake), audit, production gate (**57 pytest**), `/new`-reminder. **TUI auto `/new`** bij `gateway.ready` + live `fs.watch`. E2E **16/16**. **Consolidatie (2026-05-24):** core MEMORY/USER onder limiet (2530/1225 tekens); `deduplicate_memories.py` bugfix; `sync_profile_memories.ps1` genormaliseerde merge (seed + runtime/user-prefs). Backup: `%LOCALAPPDATA%\hermes\_memory_consolidation_backup_*`. **Obsidian L4:** `OPEN_OBSIDIAN_VAULT.bat` + scaffold. Docs: `MEMORY_ARCHITECTURE.md` (sectie Consolidatie), `TRUST_FORENSIC_PROTOCOL.md`.
