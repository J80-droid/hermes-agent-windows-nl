...
**Nous 100% intact + overlay (2026-06, productie):** Tier A strict drift `Test-NousTreeIdentical.ps1`; fork via `overlay/bootstrap.py` + runtime patches (`cli_fork_patch`, `cli_command_patches`, `cli_tps_command`, `cli_tps_stream_hooks`, `agent_throughput_fork_patch`, `pricing_fork_patch`, `models_fork_patch`); geen fork-only hooks in Tier A `cli.py` (`verify_institutional_guard.py --check-tier-a-cli`); UI-build `build_fork_ui_assets.ps1` + drift gate in CI; E2E institutional + throughput (14/14) + prompt-timer (10/10); unit `tests/overlay/` (86+); `RUN_AUDITS` overlay pytest + `-IncludeNousOverlayInstitutionalE2E`; docs `NOUS_OVERLAY_ARCHITECTURE.md`.
