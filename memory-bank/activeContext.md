...
**Memory L1–L4 productieniveau (2026-05-24):** vault = `Documents/Hermes Knowledge`; geen L3. **`SYNC_TRUST_RUNTIME.bat`** = volledige keten incl. dedup (preamble + mojibake), audit, production gate (**55 pytest**), `/new`-reminder. **TUI auto `/new`** bij `gateway.ready` + live `fs.watch`. E2E **16/16** (`RUN_MEMORY_ARCHITECTURE_E2E` → `MemoryArchitectureE2E.core.ps1`). **Obsidian L4:** `OPEN_OBSIDIAN_VAULT.bat` + scaffold `docs/templates/obsidian_vault_scaffold/` (auto via `sync_hermes_api_env.ps1`); taakbalk-`.lnk`. Docs: `MEMORY_ARCHITECTURE.md`, `TRUST_FORENSIC_PROTOCOL.md`.
