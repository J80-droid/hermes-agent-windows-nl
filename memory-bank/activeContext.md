...
**Institutionele hardening — productieniveau (2026-05-26, PASS):** `RepoHygieneCommon.ps1`; guard + QuickFix + health check; `UPDATE_HERMES.bat -QuickFix` (HERMES_WIN shift-safe); guard-log met trim; legal skills (rate limit, 2MB cap, ECLI); pytest **101** + `tests/windows/test_repo_hygiene_institutional_e2e.py` (e2e marker); E2E **14/14** hardening, **9/9** repo-hygiene, update-integratie **12/12**; `RUN_INSTITUTIONAL_PRODUCTION_GATE.bat` **PASS** (incl. hardening). Docs: `WORKSPACE_CONVENTIONS.md`, `INSTITUTIONAL_OPERATIONS.md`, README's, `AGENTS.md`.
