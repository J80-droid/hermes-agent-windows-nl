...
**Jatevo custom provider (2026-06-01):** named provider onder `providers.jatevo`; `hermes model` → key-stap (Keep/Replace/Clear), persist naar root runtime + legacy + profiel-`.env`; live `/v1/models` only (geen hardcoded modellen); base URL **`https://jatevo.ai/v1`** (niet `api.jatevo.ai`); template `docs/templates/PROVIDERS_JATEVO.yaml`; doctor/E2E/sync `JATEVO_API_KEY`.
