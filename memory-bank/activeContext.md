...
**Institutioneel 10/10 (2026-05-23, afgerond + guardrails):** palet, NFR, normalizer-pariteit, score 10/10, labels verticaal, Web live palette. **Pseudo-tabel normalizer (2026-05-23):** `ensure_markdown_table_dividers` + `normalize_pseudo_tables_to_markdown` — underscore/`A: … _____ B: …` → markdown-tabel vóór Rich-render (turn-onafhankelijk); parity Web/Ink; SOUL verbiedt pseudo-layout; guardrails in `diagnose_renderer --verify` + `score_institutional_render` (`vergelijking_tabel`); **E2E 10/10** `RUN_PSEUDO_TABLE_NORMALIZER_E2E.bat` + `scripts/verify_pseudo_table_normalizer.py --verify`. **Herstel na IDE-drift:** `APPLY_INSTITUTIONAL_RUNTIME.bat` (config + SOUL + E2E 11/11). **Preventie:** `scripts/verify_institutional_guard.py`, drift in `diagnose_renderer.py --verify`, `.cursor/rules/institutional-presentatie.mdc`, `docs/INSTITUTIONAL_PORTING_GUIDE.md`. **Na pull/update/IDE:** `/new` + rooktest; na SOUL-wijziging: `SYNC_SOUL_SNIPPETS.bat`.
