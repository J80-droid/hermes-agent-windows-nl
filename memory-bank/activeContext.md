...
**Upstream sync fase 2 + TUI layout (2026-05-25):** `Invoke-UpstreamGitMergeIfBehind` (preflight fetch-dedup, rev-parse/rev-list guards); `pip install -e .` na merge vóór `hermes update`; TUI `statusRuleMinLeftWidth` + `leftWidth` in `resolveStatusRuleLayout`; finite cols/breakdown in `usageCostBar.ts`. E2E **8/8** `RUN_UPSTREAM_SYNC_PHASE2_E2E.bat`; vitest usageCostBar **63** tests. Merge 87+17: `82e92a08b`, `9e49dbd85`; integratie `RUN_UPSTREAM_MERGE_INTEGRATION_E2E` 10/10.
...
**Memory L1–L4 productieniveau (2026-05-24):** vault = `Documents/Hermes Knowledge`; geen L3. **`SYNC_TRUST_RUNTIME.bat`** = volledige keten incl. dedup (preamble + mojibake), audit, production gate (pytest memory/trust), `/new`-reminder. **Pending trust bij start (2026-05-24):** na mislukte trust in UPDATE → `pending_trust_runtime.json`; `launch_hermes.bat` → `launch_pending_trust_runtime.ps1` → `Invoke-TrustRuntimeLight.ps1` (geen gate, slimme SOUL-skip). E2E: `RUN_PENDING_TRUST_START_E2E.bat`; pytest `test_pending_trust_runtime.py` (36 tests); `RUN_AUDITS -IncludePendingTrustStartE2E`. **TUI auto `/new`** bij `gateway.ready` + live `fs.watch`. E2E **18/18** (consolidatie-layout, legacy root, U+00A7 §-split). **Consolidatie:** `HermesMemoryMergeCommon.ps1` (merge/rebalance), `CONSOLIDATE_ROOT_MEMORIES.bat`, dedup incl. `memories/`; core ~3145 tekens (9 secties incl. MCP/multi-profile); legal domein-only (~2717). Backup: `%LOCALAPPDATA%\hermes\_memory_consolidation_backup_*`. **Obsidian L4:** `OPEN_OBSIDIAN_VAULT.bat` + scaffold. Docs: `MEMORY_ARCHITECTURE.md`, `TRUST_FORENSIC_PROTOCOL.md`.
