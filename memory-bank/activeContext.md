...
**Codebase Viz pygount pre-warm + disk-cache (2026-05-29):** `scripts/warm_codebase_viz_pygount_cache.py` + `Ensure-CodebaseVizPygountCache` in `launch_dashboard_on_start.ps1` — eerste pygount-run **vóór** dashboard-start (eenmalig ~600s); persistente cache `output/research/codebase_viz_pygount_cache.json` (hydrate na herstart, stale bij timeout); defaults **600s** timeout, **3600s** pygount-TTL, **300s** API-TTL; skip pre-warm: `HERMES_CODEBASE_VIZ_PREGOUNT_CACHE=skip`. UI-fixes + live E2E harness (`RUN_CODEBASE_VIZ_LIVE_E2E.bat`). Handmatig warmen: `python scripts/warm_codebase_viz_pygount_cache.py`.
