...
**Memory L1–L4 productieniveau (2026-05-23):** vault = `Documents/Hermes Knowledge`; geen L3. **`SYNC_TRUST_RUNTIME.bat`** = volledige keten: seed sync, dedup (hermes-env), limits, vault-env, snapshot, `audit_profile_memories`, `RUN_MEMORY_PRODUCTION_GATE`, `/new`-reminder JSON (banner in Hermes; `/new` wist vlag). Ook `POST_GIT_PULL` / `UPDATE_HERMES`. Skip pytest: `HERMES_SKIP_MEMORY_PRODUCTION_GATE=1`. Docs: `MEMORY_ARCHITECTURE.md`, `TRUST_FORENSIC_PROTOCOL.md`.
