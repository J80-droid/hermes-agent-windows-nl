...
export function ToolsetConfigDrawer({ toolset, onClose, onChanged }: Props) {
  ...
  const handleToggle = async (next: boolean) => {
    setToggling(true);
    try {
      await toolsetDashboardApi.toggleToolset(toolset.name, next);
      setEnabled(next);
      showToast(
  ...
  const handleSelectProvider = async (provider: ToolsetProvider) => {
    ...
    try {
      await toolsetDashboardApi.selectToolsetProvider(toolset.name, provider.name);
  ...
  const handleSaveKeys = async (provider: ToolsetProvider) => {
    ...
    try {
      const res = await toolsetDashboardApi.saveToolsetEnv(toolset.name, env);
  ...
  const handleRunPostSetup = async (provider: ToolsetProvider) => {
    ...
    try {
      await toolsetDashboardApi.runToolsetPostSetup(toolset.name, provider.post_setup);
