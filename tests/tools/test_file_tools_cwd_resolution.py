...
def _disable_file_sandbox_for_cwd_tests(monkeypatch):
    """Fork: cwd resolution tests use arbitrary tmp dirs; sandbox is fork-specific."""
...
def test_warning_fires_when_relative_path_escapes_workspace(_isolated_cwd, monkeypatch):
    ...
    # Windows: message uses !r paths (escaped backslashes); match by folder name.
    assert decoy.name in warn
    assert workspace.name in warn
