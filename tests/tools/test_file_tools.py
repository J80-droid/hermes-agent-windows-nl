...
def _resolved_tool_path(path: str) -> str:
    """Match ``_resolve_path_for_task`` for absolute paths (Windows-safe)."""
...
class TestWriteFileHandler:
    ...
    def test_writes_content(self, mock_get):
        ...
        mock_ops.write_file.assert_called_once_with(
            _resolved_tool_path("/tmp/out.txt"), "hello world!\n"
        )
...
class TestPatchHandler:
    ...
    def test_replace_mode_calls_patch_replace(self, mock_get):
        ...
        mock_ops.patch_replace.assert_called_once_with(
            _resolved_tool_path("/tmp/f.py"), "foo", "bar", False
        )
    ...
    def test_replace_mode_replace_all_flag(self, mock_get):
        ...
        mock_ops.patch_replace.assert_called_once_with(
            _resolved_tool_path("/tmp/f.py"), "x", "y", True
        )
