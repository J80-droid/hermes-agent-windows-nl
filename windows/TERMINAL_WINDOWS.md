...
Code: `hermes_cli/institutional_render.py`, `hermes_cli/display_markdown.py` (`get_assistant_console_theme()`), gateway `agent/rich_output.py`. TUI statusbalk-kosten: `hermes_cli/usage_snapshot.py`, `ui-tui/src/domain/usageCostBar.ts` (`resolveStatusRuleLayout` — kosten vóór cwd, niet afgekapt), `ui-tui/src/domain/liveTurnCost.ts`, `ui-tui/src/components/appChrome.tsx`; toggle `/cost`; audit `windows/audits/RUN_STATUS_BAR_COST_E2E.bat`. Klassieke CLI: eindpaneel via `ChatConsole(theme=get_assistant_console_theme())`; tijdens stream (met `streaming=false`) ruwe markdown, Rich pas op het eindpaneel.
