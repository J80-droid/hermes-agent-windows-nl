# Sync ## Advisory & trust from repo template into all profile SOUL.md files.
param(
    [string]$RepoRoot = '',
    [string]$HermesRoot = ''
)
...
$templatePath = Join-Path $RepoRoot 'docs/templates/SOUL_SHARED_ADVISORY.md'
...
foreach ($path in $targets) {
    ...
    if ($content -match '(?ms)^## Advisory & trust\s*\r?\n.*?(?=^\#\# |\z)') {
        $newContent = $content -replace '(?ms)^## Advisory & trust\s*\r?\n.*?(?=^\#\# |\z)', "$snippet`r`n`r`n"
    } elseif ($content -match '(?ms)^## Forensic & trust \(legal\)\s*\r?\n') {
        $newContent = $content -replace '(?ms)^(## Forensic & trust \(legal\)\s*\r?\n)', "$snippet`r`n`r`n`$1"
    } elseif ($content -match '(?ms)^## Outputformaat \(institutioneel\)\s*\r?\n') {
...
Write-Host "[INFO] Advisory & trust bijgewerkt: $updated SOUL-bestand(en)" -ForegroundColor Cyan
