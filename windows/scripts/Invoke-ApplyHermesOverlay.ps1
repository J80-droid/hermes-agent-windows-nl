...
$extras = Join-Path $RepoRoot 'overlay\pyproject.extras.toml'
if (-not $SkipRagExtras -and (Test-Path -LiteralPath $extras)) {
    $ragPs1 = Join-Path $RepoRoot 'windows\scripts\install_rag_extras.ps1'
...
$uiBuild = Join-Path $RepoRoot 'windows\scripts\build_fork_ui_assets.ps1'
