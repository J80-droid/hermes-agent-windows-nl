# Legal domein E2E — implementatie (dot-source of via RUN_LEGAL_DOMAIN_E2E.ps1).
# Unit tests: windows/tests/LegalDomainE2E.Unit.Tests.ps1 (geïsoleerde paden).
param(
    [string]$RepoRoot = '',
    [string]$HermesRoot = '',
    [string]$UserDataRoot = '',
...
if (-not $UserDataRoot) {
    $UserDataRoot = Join-Path $env:USERPROFILE 'data'
} else {
...
$skipPytest = ($env:HERMES_LEGAL_E2E_SKIP_PYTEST -eq '1')
$skipRooktest = ($env:HERMES_LEGAL_E2E_SKIP_ROOKTEST -eq '1')
$skipReadiness = ($env:HERMES_LEGAL_E2E_SKIP_READINESS -eq '1')
$skipPythonSteps = ($env:HERMES_LEGAL_E2E_SKIP_PYTHON_STEPS -eq '1')

function Get-LegalE2EAuditPython {
    ...
    foreach ($p in @(
        ...
        if ($p -and (Test-Path -LiteralPath $p)) {
            $py = (& $p run -n hermes-env python -c "import sys; print(sys.executable)" 2>&1 |
                Select-Object -Last 1).Trim()
            if ($py -and (Test-Path -LiteralPath $py)) { return $py }
        }
    }
    throw 'Geen hermes-env python (HERMES_AUDIT_PYTHON of conda)'
}

$failures = 0
$legalSoul = Join-Path $HermesRoot 'profiles\legal\SOUL.md'
$domainsYaml = if ($env:HERMES_DOMAINS_YAML) { $env:HERMES_DOMAINS_YAML.Trim() } else { Join-Path $UserDataRoot 'domains.yaml' }
$rawLegal = Join-Path $UserDataRoot 'raw_source_files\04_Legal_Corporate'
$summaryPath = Join-Path $UserDataRoot 'lancedb\legal\rag_ingest_run_summary.json'
...
if (-not $skipPythonSteps) {
    $python = Get-LegalE2EAuditPython
    ...
    if (-not $skipPytest) {
        ...
        Write-Host "=== 8/$totalSteps pytest legal docs (SKIP) ===" -ForegroundColor Yellow
    }
    ...
    Write-Host '=== 7-9/12 python-stappen overgeslagen (HERMES_LEGAL_E2E_SKIP_PYTHON_STEPS=1) ===' -ForegroundColor Yellow
}
...
if (-not $skipReadiness) {
    ...
    Write-Host '[SKIP] RAG readiness (HERMES_LEGAL_E2E_SKIP_READINESS=1)' -ForegroundColor Yellow
}
...
if ($skipRooktest -or $skipPythonSteps) {
    Write-Host '[SKIP] rooktest (unit skip env)' -ForegroundColor Yellow
} else {
    ...
    if (Test-Path -LiteralPath $rooktest) {
        $python = Get-LegalE2EAuditPython
        $env:HERMES_LANCEDB_PATH = Join-Path $UserDataRoot 'lancedb\legal'
