...
function New-IdentityE2EMockRuntime {
    ...
    Set-Content -LiteralPath (Join-Path $root 'profiles\core\config.yaml') -Value @'
memory:
...
Write-Host '--- MemoryIdentityRepair core: repo keten ---' -ForegroundColor Cyan
$required = @(
...
Write-Host '--- MemoryIdentityRepair core: line repair ---' -ForegroundColor Cyan
...
try {
    ...
    Add-IdentityE2EStep 'runtime repair idempotent' ((Repair-HermesRuntimeIdentity -HermesRoot $mockRoot -Quiet).FilesChanged -eq 0)

    $noCfgRoot = Join-Path $isoParent 'empty_hermes'
    New-Item -ItemType Directory -Path $noCfgRoot -Force | Out-Null
    $skipped = Repair-HermesRuntimeIdentity -HermesRoot $noCfgRoot -Quiet
    Add-IdentityE2EStep 'geen config.yaml skip' ($skipped.Skipped -eq $true)

    Write-Host '--- MemoryIdentityRepair core: post-sync integratie ---' -ForegroundColor Cyan
