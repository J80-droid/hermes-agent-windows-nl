...
5. **Python (institutioneel)** — **conda `hermes-env`** voor RAG, CLI, setup, ingest. Repo-`.venv` alleen met werkende pip én `HERMES_ALLOW_UV_VENV=1`. Kapotte `.venv` (geen pip) → automatisch `.venv.disabled-<stamp>` via `REPAIR_PYTHON.bat` / `ensure_hermes_python.ps1`. Niet de workspace-`(venv)` van Cursor gebruiken.
