...
5b. **Terminal (TUI-kleuren)** — start via `launch_hermes.bat` / `start_hermes_split.bat` → **Windows Terminal** (`wt -M`), niet handmatig in legacy `cmd` (RGB/BGR-inversie). Zie `windows/TERMINAL_WINDOWS.md`. Override: `HERMES_SKIP_WINDOWS_TERMINAL=1`.
