...
5c. **Hermes-home & secrets** — config onder `%LOCALAPPDATA%\hermes\`; oude keys kunnen in `%USERPROFILE%\.hermes\.env` staan. Bij Gemini HTTP 400: `SYNC_HERMES_API_ENV.bat`. `apply_team_display.ps1` zet display op **alle** profielen (`profiles\*\config.yaml` via YAML-patch); model blijft in root `config.yaml`.
