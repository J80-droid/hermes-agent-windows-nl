...
5c. **Hermes-home & secrets** — config onder `%LOCALAPPDATA%\hermes\`; oude keys kunnen in `%USERPROFILE%\.hermes\.env` staan. Bij Gemini HTTP 400 of verkeerd vault-pad: `SYNC_HERMES_API_ENV.bat` (API + `OBSIDIAN_VAULT_PATH` naar alle profielen). Vault: `Documents\Hermes Knowledge` — zie `docs/MEMORY_ARCHITECTURE.md`. `apply_team_display.ps1` zet display op **alle** profielen; model blijft in root `config.yaml`.
