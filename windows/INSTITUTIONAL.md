...
5. **Python (institutioneel)** — **conda `hermes-env`** voor RAG, CLI, setup, ingest, **en IDE** (`.vscode/settings.json` via `sync_hermes_ide_python.ps1` / `REPAIR_PYTHON.bat`). **Native invoke:** `HermesNativeInvoke.ps1`. Repo-`.venv` niet als runtime; quarantaine via `ensure_hermes_python.ps1`. Optioneel uv alleen met `HERMES_ALLOW_UV_VENV=1`. Zie `docs/HERMES_START.md` § Python institutioneel.
