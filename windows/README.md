...
| Domein-toolsets (mcp+kanban standaard) | `SYNC_DOMAIN_TOOLSETS.bat` — manifest → profielen; **overslaat** profielen met `platform_toolsets._user_customized.cli` (na `hermes -p <naam> tools`); reset: `--force-manifest`; zie `docs\DOMAIN_TOOLSET_AUDIT.md` |
...
| Legal productie-poort | `docs\LEGAL_PRODUCTION_GATE.md` · `VERIFY_LEGAL_RUNTIME.bat` · `audits\RUN_LEGAL_PRODUCTION_E2E.bat` (17) · `audits\RUN_LEGAL_MEMORY_LANGUAGE_LAYERS_E2E.bat` (EN trust + 3× NL USER, 9) · `audits\RUN_LEGAL_PROACTIVE_SPARRING_E2E.bat` (19; auto via `Invoke-LegalProactiveSparringE2E.ps1`) · `windows\audits\RUN_LEGAL_DOMAIN_E2E.bat` (runtime 12) |
