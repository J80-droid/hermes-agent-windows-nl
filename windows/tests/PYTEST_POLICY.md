...
**Windows CI collectie:** `pytest_fork_gate.yaml` → `upstream.ignores` sluit o.a. `tests/acp`, `tests/gateway` en Unix-only modules uit (geen volledige upstream Linux-stack op `windows-latest`).
